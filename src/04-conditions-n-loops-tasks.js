...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < splitPaths[0].length; i += 1) {
    const directory = splitPaths[0][i];
    if (splitPaths.every((path) => path[i] === directory)) {
...
function evaluateTicTacToePosition(position) {
  const lines = [
    ...
    [position[0][0], position[1][1], position[2][2]], // Diagonal 1
    [position[0][2], position[1][1], position[2][0]], // Diagonal 2
  ];

  // Use `find` to get the first winning line
